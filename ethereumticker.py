...
class EthereumTicker(Boardlet):
  def __init__(self, parent, targetCurr):
    super(EthereumTicker, self).__init__(parent)
    self.p_model = Ethereum( targetCurr )
    self.initUI()

  def initUI(self):
    super(EthereumTicker, self).initUI()
    self.p_icon = QtGui.QLabel(self)
    self.p_icon.setGeometry( self.b_imgx(), self.b_imgy(),
                             self.b_iconwidth(),self.b_iconheight() )
    self.p_icon.setPixmap( QtGui.QPixmap(os.getcwd() + "/img/ethereum.png" ) )
  ...
  def paintEvent(self, e):
    super(EthereumTicker, self).paintEvent(e)
...
class Ethereum(Modellet):
  def __init__(self, targetCurr='ETH'):
  ...
  def doRefresh(self):
    ...
    req = urllib2.Request( 'https://bittrex.com/api/v1.1/public/getticker?market=BTC-ETH', None, headers )

    try:
      resp = urllib2.urlopen(req).read()
      self.p_bestBid = str( json.loads(resp)['result']['Bid'] )
      self.p_bestAsk = str( json.loads(resp)['result']['Ask'] )
      super(Ethereum, self).setFaultFlag(False)
      super(Ethereum, self).setLastUpdatedNow()
    except Exception:
      exc_type, exc_value, exc_traceback = sys.exc_info()
      lines = traceback.format_exception(exc_type, exc_value, exc_traceback)
      print ''.join('!! ' + line for line in lines)
      super(Ethereum, self).setFaultFlag(True)
